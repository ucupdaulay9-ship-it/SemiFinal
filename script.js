...
window.addEventListener('load', function() {
    map = L.map('map-interaktif').setView([3.3223367, 99.1622936], 16);
    ...
    fetch(`https://nominatim.openstreetmap.org/search?format=json&limit=1&q=${encodeURIComponent(query)}`)
        ...
        .then(data => {
            if(data && data.length > 0) {
                ...
                if(tipe === 'jemput') { markerJemput.setLatLng([lat, lon]); map.setView([lat, lon], 16); } 
                else { markerTujuan.setLatLng([lat, lon]); map.setView([lat, lon], 16); }
                kalkulasiJarakLangsung();
...
function perbaruiDaftarTarifArmada(jarakKm) {
    ...
    // Tarif masing-masing kendaraan
    const tarifPickup = 22000 + (jarakKm * 5000); 
    const tarifBox = 28000 + (jarakKm * 7000); 
    const tarifTruk = 33000 + (jarakKm * 10000);

    // Update untuk Mobil Pick-Up
    document.getElementById('tarif-pickup').innerText = formatRupiah(tarifPickup); 
    document.getElementById('waktu-pickup').innerText = durasiWaktuTeks;
    document.getElementById('jarak-pickup').innerText = jarakKm + " km"; // Baris baru untuk Jarak

    // Update untuk Mobil Box
    document.getElementById('tarif-box').innerText = formatRupiah(tarifBox); 
    document.getElementById('waktu-box').innerText = durasiWaktuTeks;
    document.getElementById('jarak-box').innerText = jarakKm + " km"; // Baris baru untuk Jarak

    // Update untuk Mobil Truk
    document.getElementById('tarif-truk').innerText = formatRupiah(tarifTruk); 
    document.getElementById('waktu-truk').innerText = durasiWaktuTeks;
    document.getElementById('jarak-truk').innerText = jarakKm + " km"; // Baris baru untuk Jarak
}
